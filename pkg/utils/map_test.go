...
func Test_ToParam(t *testing.T) {
	...
	assert.Nil(err)
	r := ToParam(u)
	assert.Equal(r, map[string]string{"f": "3", "fenlei": "256", "https://www.baidu.com/s?ie": "utf-8", "inputT": "4905", "prefixsug": "docker%20env", "rqlang": "cn", "rsp": "0", "rsv_bp": "1", "rsv_btype": "i", "rsv_dl": "ts_0", "rsv_enter": "1", "rsv_idx": "1", "rsv_pq": "e37e3569000003df", "rsv_sug1": "10", "rsv_sug2": "1", "rsv_sug3": "10", "rsv_sug4": "4904", "rsv_sug7": "100", "rsv_t": "bad0PtT22fO15WdADr1HsPCYvpKKXDNWfD77qwvYtJ0JjUrF+vQPrcRAEqY", "tn": "baidu", "wd": "docker environment设置环境变量"})
}

func Test_Strings(t *testing.T) {
	assert := assert.New(t)
	r := Strings([]interface{}{"aa", "bb", "cc", "dd", "ee"})
	assert.Equal(r, map[string]bool{"aa": true, "bb": true, "cc": true, "dd": true, "ee": true})
}

func Test_ToValues(t *testing.T) {
	assert := assert.New(t)
	r := ToValues(map[string]string{"aa": "bb"})
	assert.Equal(r, url.Values{"aa": []string{"bb"}})
}

func Test_ToMapStrings(t *testing.T) {
	assert := assert.New(t)
	r := ToMapStrings(map[string]interface{}{"aa": "11", "bb": "cc"})
	assert.Equal(r, map[string]string{"aa": "11", "bb": "cc"})
