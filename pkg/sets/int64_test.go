...
func TestInt64Set(t *testing.T) {
	...
	assert.Equal(s.Intersection(s2).List(), []int64{2}) // string not order
	assert.Equal(s.Intersection(s2).UnsortedList(), []int64{2})
	ret, ok := s.Intersection(s2).PopAny()
	assert.Equal(ret, int64(2))
	assert.True(ok)
	...
	s3 := NewInt64(1, 2, 3)
	s4 := Int64KeySet(map[int64]interface{}{1: "", 2: "", 3: ""})
